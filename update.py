...
entry_typeofid=tk.Entry(frame1,font=20,bd=5,relief='sunken')
...
def update_func():
    ...
    try:
        ...
        main1.commit()
        sql="Update bookinghotel set cust_name=%s, address=%s, dob=%s, mobilenum= %s, email=%s, nationality=%s, gender=%s ,checkin=%s, checkout=%s, typeofid=%s, roomno=%s where cust_id=%s"
        mycursor.execute(sql,(custname,address,dob,mobile,email,nationality,gender,checkindate,checkoutdate,typeofid,roomnumber,custid))
        main1.commit()
...
update_button=tk.Button(frame2,text="UPDATE DATA",relief="raised",font=10,command=update_func)
update_button.place(relx=0.32,rely=0.2,relwidth=0.35,relheight=0.15)

reset_button=tk.Button(frame2,text="RESET",relief="raised",font=10,command=reset)
reset_button.place(relx=0.32,rely=0.36,relwidth=0.35,relheight=0.15)

exit_button=tk.Button(frame2,text="EXIT",relief="raised",font=10,command=exitbut)
